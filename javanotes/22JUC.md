...
**面试官**‌：请解释AQS的原理。
‌**回答**‌：
AQS是Java并发包中构建锁和同步器的核心框架，通过一个双向队列管理等待线程，用volatile的state变量控制资源状态。它提供独占和共享两种模式，子类只需实现`tryAcquire`/`tryRelease`等方法定义资源获取和释放逻辑，而线程排队、阻塞唤醒等底层操作由AQS完成。例如，ReentrantLock的非公平锁在加锁时直接尝试修改state，若失败则加入队列等待；而Semaphore通过state表示剩余许可数，允许多线程共享资源。AQS的灵活设计使得开发者能够高效构建复杂的同步工具。

### **AQS的应用场景**‌

| 同步工具类                 | AQS实现原理                                                  |
| -------------------------- | ------------------------------------------------------------ |
| ‌**ReentrantLock**‌          | 独占模式，state表示重入次数。                                |
| ‌**Semaphore**‌              | 共享模式，state表示剩余许可数。                              |
| ‌**CountDownLatch**‌         | 共享模式，state表示计数器值，减到0时唤醒。                   |
| ‌**ReentrantReadWriteLock**‌ | 读锁（共享）+ 写锁（独占），state高16位为读锁，低16位为写锁。 |

...
- 索引下推核心就是不着急回表，先判断条件是否成立，以减少需要索引的记录，减少回表次数，提高效率。比如上面先判断大于Z，再回表后判断是否以A结尾。可是优化后就先判断大于Z，再判断是否以A结尾，满足条件的才进行回表，减少了需要回表的行数，这就是索引下推。⚠️⚠️⚠️
...
tail /var/lib/mysql/atguigu-slow.log -n 100
```

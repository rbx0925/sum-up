...
import com.ikang.idata.common.entity.vo.DeferredVo;
import com.ikang.idata.search.search.service.DeferredService;
...
@WebMvcTest(DeferredController.class)
class DeferredControllerTest {
    ...
    @MockBean
    private DeferredService mockDeferredService;

    @Test
    void testFind() throws Exception {
        // Setup
        // Configure DeferredService.find(...).
        ...
        when(mockDeferredService.find(any(DeferredVo.class))).thenReturn(baseSearch);

        // Run the test
        final MockHttpServletResponse response = mockMvc.perform(post("/deferred/find")
    ...
    void testFindGroupBy() throws Exception {
        // Setup
        // Configure DeferredService.findGroupBy(...).
        final BaseSearch baseSearch = new BaseSearch();
        baseSearch.setPageSize(0);
        baseSearch.setPageNum(0);
        baseSearch.setReturnData(Arrays.asList(new HashMap<>()));
        ...
        baseSearch.setDesensitization(false);
        baseSearch.setSort("sort");
        when(mockDeferredService.findGroupBy(any(DeferredVo.class))).thenReturn(baseSearch);

        // Run the test
        final MockHttpServletResponse response = mockMvc.perform(post("/deferred/findGroupBy")
                .content("content").contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();
    ...
    void testExportExcel() throws Exception {
        // Setup
        // Run the test
        final MockHttpServletResponse response = mockMvc.perform(post("/deferred/path")
                .content("content").contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
                .andReturn().getResponse();

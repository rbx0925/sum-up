...
public class DepartmentRankServiceTest {
    ...
    void testFind() throws Exception {
        ...
        when(mockAuthorityFeignService.getChannelOrHospitals(0L))
                .thenReturn(userChannelAndHospital);
        ...
        when(mockReturnDataFeignService.listByReturnDataIndex("reader_department"))
    ...
    public void testDSL() {
        ...
        //项目名称
        TermsAggregationBuilder itemNameField = AggregationBuilders
                .terms(ReaderEnum.item_name.getFields())
                .field(ReaderEnum.item_name.getFields())
                .size(10000)
                .executionHint("map");
        SumAggregationBuilder aggregationSum = AggregationBuilders
                .sum(ReaderEnum.ct.getFields())
                .field(ReaderEnum.ct.getFields());
        TopHitsAggregationBuilder resultAgg = AggregationBuilders
                .topHits("result")
                .size(1);
    ...
    private void setParams(DepartmentFilmVo departmentFilmVo, List<QueryBuilder> must, UserChannelAndHospital channelOrHospitals) {
        ...
        if (StringUtils.isNotBlank(departmentFilmVo.getInstituteId())) {
            ...
            List<String> collectCheckLineCode = channelOrHospitals
                    .getHospitals()
                    .values()
                    .stream()
                    .flatMap(List::stream)
                    .map(UserHospitalAndName::getCheckLines)
                    .flatMap(List::stream)
                    .map(UserCheckLineAndName::getCheckLineCode)
                    .collect(Collectors.toList());
            Optional<String> any = list
                    .stream()
                    .filter(s -> !collectCheckLineCode.contains(s))
                    .findAny();
            if (any.isPresent()) {
    ...
    public void testDSLT() {
        ...
        setParams(departmentFilmVo, must, channelOrHospitals);
        builder.query(queryBuilder);
        TermsAggregationBuilder aggDoctorField = AggregationBuilders
                .terms(ReaderEnum.agg_doctorSWorkload.getFields())
                .field(ReaderEnum.agg_doctorSWorkload.getFields())
                .size(10000)
                .executionHint("map");
        SumAggregationBuilder aggregationSum = AggregationBuilders
                .sum(ReaderEnum.ct.getFields())

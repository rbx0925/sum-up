...
public final class LockUtil {
     ...
     * @param l
     */
    public static void setLocker(Locker l) {
        locker = l;
    }
...
public class CustomizeMetaObjectHandler implements MetaObjectHandler {
    ...
    public void insertFill(MetaObject metaObject) {
        log.info("插入填充................");
        this.strictInsertFill(metaObject, CREATEBY, Long.class, UserUtil.getLoginId());
        this.strictInsertFill(metaObject, CREATETIME, LocalDateTime.class, LocalDateTime.now());

        this.strictUpdateFill(metaObject, UPDATEBY, Long.class, UserUtil.getLoginId());
...
@Configuration
@MapperScan("com.ikang.idata.mapper*")
...
```java
//启用缓存，在类上方标注注解@EnableCaching配置springboot程序中可以使用缓存
@Configuration
@EnableCaching
...
public class RedisCache {
	...
	public <T> T getByJsonSer(String key, Class<T> clazz) {
		return get(key, (value) -> JSONObject.parseObject(value, clazz));
	}
...
public class RestTemplateConfig {
    ...
    @Bean(OK_HTTP_3_REST_TEMPLATE_SCRIPT_AGG)
    public RestTemplate okhttp3ResttemplateScript(){
        Dispatcher dispatcher = new Dispatcher();
        dispatcher.setMaxRequests(200);
        dispatcher.setMaxRequestsPerHost(50);
        OkHttpClient client = new OkHttpClient.Builder()
        ...
        OkHttp3ClientHttpRequestFactory factory = new OkHttp3ClientHttpRequestFactory(client);
        //  单位毫秒  10*60秒
        factory.setConnectTimeout(10*60000);
        factory.setReadTimeout(10*60000);
        return new RestTemplate(factory);
    }
}
```
...
public class HttpClient {
    ...
    public static final String OK_HTTP_3_REST_TEMPLATE = "okhttp3Resttemplate";
    public static final String OK_HTTP_3_REST_TEMPLATE_SCRIPT_AGG = "okhttp3ResttemplateScript";


    public static JSONObject esRequest(String url, String context) {
        log.info("esRequest::url = [{}], context = [{}]", url, context);
        HttpHeaders requestHeaders = new HttpHeaders();
        requestHeaders.add(HttpHeaders.CONTENT_TYPE, HEADER_VALUE);
        HttpEntity<String> entity = new HttpEntity<>(context, requestHeaders);
        ResponseEntity<JSONObject> mapResponseEntity;
        Instant start = Instant.now();
        try {
            mapResponseEntity = ((RestTemplate) SpringUtil.getBean(OK_HTTP_3_REST_TEMPLATE)).postForEntity(url, entity, JSONObject.class);
            log.info("------------大数据 请求用时:{}ms-----------------", Duration.between(start, Instant.now()).toMillis());
        } catch (RestClientException e) {
            log.error("2c_idata_big_data_interface_failed");
            log.error("大数据接口异常:" + url);
            log.error(e.getMessage(), e);
            throw new BusinessException("大数据接口异常:" + url);
        }
        JSONObject jsonObject = mapResponseEntity.getBody();
        //log.info("url = [{}] RequestResult:[{}]", url, jsonObject);
        if (Objects.isNull(jsonObject)) {
            throw new BusinessException("请求失败!请稍后重试!" + url);
        }
        if (!HTTP_OK.equals(jsonObject.getString(CODE))) {
            throw new BusinessException(jsonObject.getString(MESSAGE) + url);
        }
        return jsonObject;
    }


    public static JSONObject esRequestAgg(String url, String context) {
        log.info("esRequest::url = [{}], context = [{}]", url, context);
        HttpHeaders requestHeaders = new HttpHeaders();
        requestHeaders.add(HttpHeaders.CONTENT_TYPE, HEADER_VALUE);
        HttpEntity<String> entity = new HttpEntity<>(context, requestHeaders);
        ...
        Instant start = Instant.now();
        try {
            mapResponseEntity = ((RestTemplate) SpringUtil.getBean(OK_HTTP_3_REST_TEMPLATE_SCRIPT_AGG)).postForEntity(url, entity, JSONObject.class);
            log.info("------------大数据 请求用时:{}ms-----------------", Duration.between(start, Instant.now()).toMillis());
        } catch (RestClientException e) {
            log.error("2c_idata_big_data_interface_failed");
            log.error("大数据接口异常:" + url);

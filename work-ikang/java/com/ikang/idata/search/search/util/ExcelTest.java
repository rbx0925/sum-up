...
import com.ikang.idata.search.search.DemoTest;
...
//
        ...
        List<String> strings = Arrays.asList("1", "2", "null");
        //挺好
        strings.stream().map(Longs::tryParse).filter(Objects::nonNull).forEach(System.out::println);


        ClassPath from = ClassPath.from(Thread.currentThread().getContextClassLoader());
        from.getAllClasses().stream().forEach(System.out::println);
        System.out.println(from.getAllClasses().stream().count());
    }
